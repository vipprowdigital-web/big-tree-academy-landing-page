...
export default function Services() {
  ...
  return (
    <section className="w-full bg-primary text-primary py-24 pl-4 sm:pl-8 lg:pl-16 pr-0 overflow-hidden relative selection:bg-primary selection:text-secondary">
      {/* Font Sheet */}
      <style jsx global>{`
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-4 flex flex-col items-start text-left gap-4 pr-4 sm:pr-8 lg:pr-0">
          <div className="flex items-center gap-3 text-[10px] font-sans-clean uppercase font-bold tracking-[0.26em] text-secondary/80">
            <span className="w-7 h-px bg-secondary/50" />
            The Big Tree Difference
          </div>

          <h2 className="font-serif-editorial text-[clamp(2.5rem,4.5vw,3.8rem)] font-light leading-[1.1] tracking-tight text-secondary">
            Why Choose <br />
          ...
          <p className="text-xs md:text-sm font-sans-clean font-light leading-[1.8] max-w-sm mt-1 text-secondary/70">
          ...
          <div className="flex items-center gap-3 mt-6 font-sans-clean">
            <button
...
function ServiceCard({ service, index }) {
  ...
  return (
    <div className="group relative flex flex-col justify-between items-start p-8 rounded-2xl transition-all duration-500 ease-out h-77.5 bg-secondary border border-primary/10 shadow-[0_4px_24px_rgba(0,0,0,0.3)] hover:border-primary hover:shadow-[0_12px_36px_rgba(var(--primary-rgb),0.1)]">
      ...
      <div className="w-full flex justify-between items-start">
        <div className="p-3 bg-white/2 border border-white/5 rounded-xl group-hover:scale-105 transition-transform duration-500 ease-out text-primary">
          {service.icon}
        </div>

        <span className="font-serif-editorial text-xl font-light italic transition-colors duration-500 select-none opacity-20 group-hover:opacity-50 text-primary">
          -{displayIndex}-
        </span>
      </div>

      {/* Card Content Data */}
      <div className="flex flex-col gap-2 font-sans-clean w-full mt-auto">
        <h3 className="font-bold text-lg tracking-tight transition-colors duration-300 text-primary uppercase">
          {service.title}
        </h3>

        <div className="h-px group-hover:w-12 transition-all duration-500 ease-out my-1 w-6 bg-primary/40" />

        <p className="text-xs font-normal leading-relaxed tracking-wide text-primary/60">
          {service.description}
        </p>
      </div>
